...
export const generateSalesCombinedPDF = async (
  ...
  try {
    ...
    bills.forEach((bill) => {
      ...
      doc.text(`${t('addBuyer', lang).replace("New ", "").toUpperCase()}:  ${session.buyer_name || 'Unknown'}`, 12, y + 34)
      ...
      doc.text(`Invoice Amount: Rs ${formatInr(bill.grandTotal || 0)}`, 12, y + 41)
      ...
      } else if ((session.partial_payment || 0) > 0) {
    ...
    if (action === 'download') {
      doc.save(`SalesCombinedBill_${(session.buyer_name || 'Buyer').replace(/\s+/g, '_')}_${session.date || 'date'}.pdf`)
